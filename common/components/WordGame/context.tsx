...
import init, { get_feedback, get_suggestion, get_word } from "../../WASM/wordGame/rust_word_game";
...
export const WordGameContextProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    const useJS = false;
    const [difficulty, setDifficulty] = useState<Difficulty>('easy');
    const [inGame, setInGame] = useToggle(false);
    const [word, setWord] = useState<string>('');
    const totalGuesses = useMemo(() => difficulty === 'easy' ? 10 : difficulty === 'medium' ? 7 : 5, [difficulty]);
    const [gameOver, setGameOver] = useToggle(false);
    const [won, setWon] = useToggle(false);
    const [suggestion, setSuggestion] = useState<Suggestion>({ suggestion: '', probability: 0 });
    const { array: guesses, push: pushGuess, set: setGuesses } = useArray<Guess>([]);
        ...
        const validAnswers = guesses.reduce((answers, guess): string[] => {
            ...
            guess.feedback.forEach((feedback, indx) => {
                if (feedback.in_correct_location) {
                    possibleAnswers = possibleAnswers.filter((answer) => answer[indx] === feedback.letter);
                    return;
                }
                if (feedback.in_word) {
                    possibleAnswers = possibleAnswers.filter((answer) => answer.includes(feedback.letter) && answer[indx] !== feedback.letter);
                    return
                }
                possibleAnswers = possibleAnswers.filter((answer) => !answer.includes(feedback.letter));
            });
    ...
    const getSuggestionWS = async (): Promise<Suggestion> => {
        await init();
        const suggestion = JSON.parse(await get_suggestion(JSON.stringify(guesses)));
        return suggestion;
    };
    ...
    const getSuggestion = async (): Promise<void> => {
        const currSuggestion: Suggestion = useJS ? getSuggestionJS() : await getSuggestionWS();
    ...
    const submitGuess = async (guess: string) => {
        ...
        const feedback: Feedback[] = useJS ? handleGuessJS(guess) : await handleGuessWS(guess);
    ...
    const value: IWordGameContext = useMemo(() => ({
        inGame, newGame, endGame, suggestion, setDifficulty, submitGuess, guesses, won, gameOver, wordLength: word.length, guessesRemaining: totalGuesses - guesses.length
    }), [inGame, newGame, endGame, suggestion, setDifficulty, submitGuess, guesses, won, gameOver]);
