...
export default function Puzzle({ puzzle: basePuzzle, updateProgress }: PuzzleProps) {
    const { array: puzzle, set: setPuzzle, update: updatePuzzle } = useArray<number>([]);
    ...
    const Rows = useMemo(() => {
        ...
        for (let row = 0; row < 9; row++) {
            ...
            for (let cell = 0; cell < 9; cell++) {
                ...
                cells.push(<Cell key={indx} indx={indx} puzzle={puzzle} value={value} locked={locked} onChange={handleChange} />);
            }
            rows.push(<div key={row} className="sudoku-row">{cells}</div>);
        }
        return rows;
    }, [puzzle]);
